  ...
  <div class="app-container">
    ...
    <el-card shadow="never">
      ...
      >
        ...
        <el-table-column label="组件路径" align="left" width="250" prop="componentPath" />
...
function handleQuery() {
  ...
  MenuAPI.getList(queryParams)
    .then((data) => {
      menuTableData.value = arrayToTree(data);
    })
    .finally(() => {
      loading.value = false;
    });
...
function handleOpenDialog(parentId?: number, menuId?: number) {
  const data = JSON.parse(JSON.stringify(menuTableData.value));
  deleteItem(data);
  ...
  if (menuId) {
    ...
    MenuAPI.getFormData(menuId).then((data) => {
      ...
      initialMenuFormData.value = { ...params };
