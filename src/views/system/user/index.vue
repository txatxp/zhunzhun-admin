      ...
      <el-col :lg="24" :xs="24">
        ...
        <el-card shadow="never">
          ...
          <el-table v-loading="loading" :data="pageData" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="角色" width="200" align="center" prop="roleIds" />
            <!-- <el-table-column label="角色码" width="200" align="center" prop="roleNames" /> -->
            <el-table-column label="性别" width="100" align="center" prop="sex">
              <template #default="scope">
                {{ scope.row.sex == 1 ? "男" : "女" }}
              </template>
            </el-table-column>
            <!-- <el-table-column label="部门" width="120" align="center" prop="department" /> -->
...
async function handleOpenDialog(id?: number) {
  dialog.visible = true;
  const role: any = await RoleAPI.getOptions();
  roles.value = role.list;
  // 加载角色下拉数据源
  roleOptions.value = role.list;

  if (id) {
    dialog.title = "修改用户";
    UserAPI.getFormData(id).then((data) => {
      console.log(data, "----------data");
      const arr = data.roleIds.split(",").map((item) => Number(item));
      const copy = {
...
const handleSubmit = useDebounceFn(() => {
  userFormRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      // 新增用户
      let roleIds = formData.roleIds.join(",");
      // 角色码
      let roleCodes = roles.value
        .filter((item) => roleIds.includes(item.id))
        .map((item) => item.code);
      // 角色名
      let roleNames = roles.value
        .filter((item) => roleIds.includes(item.id))
        .map((item) => item.name);
      // 组装数据
      const data = {
        ...
        sex: formData.gender ? Number(formData.gender) : 1,
      ...
      RoleAPI.getPageRole({ arr: formData.roleIds }).then((res) => {
        // 多个角色
        let arr: any = [];
        if (res && res.length > 0) {
          res.forEach((item) => {
            let _res = item.menuIds.replace(/\\/g, "");
            arr.push(JSON.parse(_res));
          });
        }
        ...
        RoleAPI.getFormData2({ arr: result }).then((res2: any) => {
          const arr2 = <any>[];
          res2.forEach((item: any) => {
            arr2.push(item.perm);
          });
          data.perms = arr2.join(",");
          if (userId) {
