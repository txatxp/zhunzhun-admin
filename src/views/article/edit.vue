<template>
  <div class="article">
    ...
    >
      ...
      <el-form-item label=" ">
        <el-button type="primary" @click="submit(articleTypeAddRef)">确定修改文章</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
const submit = async (formEl: any) => {
  await formEl.validate((valid: any, fields: any) => {
    if (valid) {
      ...
      ArticleAPI.updateArticle(route.query.id, form.value).then((res: any) => {
        if (res.code == 200) {
...
onMounted(() => {
  ...
  if (route.query.id) {
    ...
    ArticleAPI.getArticleById(route.query.id).then((res: any) => {
      if (res.code == 200) {
        const data = res.data;
